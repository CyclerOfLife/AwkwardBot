...
export default class MissingPermissionsListener extends Listener {
	...
	public async exec (message: Message, command: Command, type: string, missing: string | string[]): Promise<any> {
		...
		switch (missing) {
			case 'nsfw':
				return message.util!.send(
					new MessageEmbed().setDescription(`This channel is not \`NSFW\`. Please enter a \`NSFW\` channel.`),
				);
				break;
		}
	}
	...
	public missingPermissions (user: any, permissions: any) {
		const result = user.permissions
			.missing(permissions)
			.map(
				(str) => `\`${str.replace(/_/g, ' ').toLowerCase().replace(/\b(\w)/g, (char) => char.toUpperCase())}\``,
			);

		return
			result.length > 1 ? `${result.slice(0, -1).join(', ')} and ${result.slice(-1)[0]}` :
			result[0];
	}
}
